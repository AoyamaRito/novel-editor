...
let conn = {}; // 連接モデル(conn.json): 品詞bigram接続コスト+著者表記bigram+かな嗜好+語→品詞。無ければ従来動作
...
let beginner = localStorage.getItem('ne:beginner') === 'on'; // 初心者モード: 現在行を横一行で大きく+盤をデカく(学習用)
...
let spreadCache = { key: '', list: [] };
function spreadsOfCurrent() { // 現在話の見開き境界(組版エンジン共用なので表示と完全一致)
  const key = (curName || curDocId) + '|' + text.length;
  if (spreadCache.key === key) return spreadCache.list;
  const tokens = [...text].map((c, i) => ({ c, i }));
  const lines = layoutLines(tokens);
  const list = [];
  for (let p = 0; p < lines.length; p += PAGE_LINES * 2) {
    const first = lines.slice(p).flat().find((t) => t.i != null && t.c !== '⏎');
    if (!first) break;
    const snippet = text.slice(first.i).split('\n').find((l) => l.trim())?.trim().slice(0, 12) || '';
    list.push({ start: first.i, snippet });
  }
  spreadCache = { key, list };
  return list;
}
function refreshToc() { // 目次(クリックで話を切替+要約/一行目)。現在話は打鍵のたびにライブ更新
  const toc = document.getElementById('toc');
  if (!toc) return;
  if (!ledger) { // 作品フォルダを開いていなくても、いまの原稿の見開き目次は出す
    const sps = spreadsOfCurrent();
    const subs = sps.length >= 2 ? sps.map((sp, i) => `<div class="sp" data-pos="${sp.start}" data-sp="${i}">${i + 1}　${sp.snippet.replace(/</g, '&lt;')}…</div>`).join('') : '';
    toc.innerHTML = `<div class="ep cur"><div class="eprow"><span>${curDocId.replace('novel:', '')}</span><span class="len">${text.length}字</span></div>${subs}</div>`;
    return;
  }
  toc.innerHTML = ledger.order.map((n) => {
    ...
    let subs = '';
    if (cur) { // 本文の大きさに応じて粒度を細かく: 見開き2つ以上に育ったらサブ目次
      const sps = spreadsOfCurrent();
      if (sps.length >= 2)
        subs = sps.map((sp, i) => `<div class="sp" data-pos="${sp.start}" data-sp="${i}">${i + 1}　${sp.snippet.replace(/</g, '&lt;')}…</div>`).join('');
    }
    return `<div class="ep${cur ? ' cur' : ''}" data-ep="${n.replace(/"/g, '&quot;')}"><div class="eprow"><span>${n.replace(/\.txt$/, '')}</span><span class="len">${len !== '' ? len + '字' : ''}</span></div>${sum}${subs}</div>`;
  }).join('');
}
...
globalThis.__neBeginner = { toggle: toggleBeginner, on: () => beginner }; // e2e 用
function toggleMemo() {
  memoOpen = !memoOpen;
...
function renderBeginner(el) { // 現在行だけを横一行で大きく(学習用フォーカス表示)
  el.classList.remove('tate', 'ov', 'tut');
  el.classList.add('beginner-line');
  const ls = text.lastIndexOf('\n', cursor - 1) + 1;        // 現在行の先頭
  let le = text.indexOf('\n', cursor); if (le < 0) le = text.length; // 現在行の末尾
  const pendM = text.slice(committedTo, cursor).match(/[ぁ-んー]+$/);
  const pendStart = Math.max(ls, cursor - (pendM ? pendM[0].length : 0));
  const pr = predict();
  el.innerHTML =
    `<div class="bline">` +
    esc(text.slice(ls, pendStart)) +
    (pendStart < cursor ? `<span class="pend">${esc(text.slice(pendStart, cursor))}</span>` : '') +
    composingHtml() + '<span class="caret"></span>' +
    (pr ? `<span class="ghost">${esc(pr.ghost)}</span>` : '') +
    esc(text.slice(cursor, le)) +
    `</div>`;
  // キャレットを横方向の中央へ(行が画面より長い時のスクロール)
  const caretEl = el.querySelector('.caret');
  if (caretEl && typeof caretEl.offsetLeft === 'number' && followCaret) {
    el.scrollLeft = Math.max(0, caretEl.offsetLeft - el.clientWidth * 0.5);
  }
  document.getElementById('mode').textContent = mode === 'CAND' ? '▼' : '初';
  document.getElementById('count').textContent = `${text.length}字 ・ 初心者モード`;
}
...
function lookup(yomi, ctx = '') {
  ...
  if (conn.kana && conn.kana[yomi]) score[yomi] = (score[yomi] || 0) + 500; // 著者が高頻度でかな表記する語(できる/いる等)はかな形を優先。user/ctxの確定には負ける
...
const CC_W = 0.85;   // 接続コストの重み(助詞飛ばし等の不自然な並びを抑える)
function posOf(yomi, surf) { // 候補の品詞: 語→品詞(厳密) → 読み→品詞(多数決) → 推定
  const w = conn.wpos && conn.wpos[yomi + SEP + surf];
  if (w) return w;
  const y = conn.ypos && conn.ypos[yomi];
  if (y) return y;
  return '名詞:一般'; // 未知語は名詞扱い(基底辞書は名詞・固有名詞が大半)
}
function connCost(prevPos, pos) { // 品詞bigram接続コスト(低いほど自然な並び)
  const bi = conn.posBi;
  if (!bi) return 0;
  const row = bi[prevPos];
  if (!row) return DEF_CC;
  return row[pos] != null ? row[pos] : DEF_CC;
}
// 頻度校正: 観測回数が多い表記ほどコストを下げる(同一層内の順位付け)。[lo,hi] に収める
const fcost = (count, hi, lo) => Math.max(lo, Math.round(hi - (hi - lo) * Math.min(1, Math.log(count + 1) / Math.log(40))));
function wordCands(sub) {
  const res = [];
  const push = (s2, cost) => res.push([s2, cost, posOf(sub, s2)]);
  const u = userDict[sub];
  if (u) Object.entries(u).sort((a, b) => b[1] - a[1]).slice(0, 2).forEach(([s2, n]) => push(s2, fcost(n, 70, 30))); // 自分の確定=最安
  const a2 = autoDict[sub];
  if (a2) Object.entries(a2).sort((x, y) => y[1] - x[1]).slice(0, 2).forEach(([s2, n]) => push(s2, fcost(n, 110, 60))); // 自動登録の固有名詞
  (dict[sub] || []).slice(0, 3).forEach(([s2, n]) => push(s2, fcost(n, 175, 90))); // 自己コーパス(頻度校正)
  (baseDict[sub] || []).slice(0, 3).forEach(([s2, c]) => push(s2, c)); // 基底はmozcコスト
  const seen = new Set();
...
function latticeBest(run, K = 8, ctx = '', con = null) {
  ...
  for (let i = 1; i <= n; i++) {
    ...
    for (let j = Math.max(0, i - 12); j < i; j++) {
      ...
      else if (conn.kana && conn.kana[sub]) kc = Math.max(40, 125 - conn.kana[sub]); // かな嗜好(著者統計)
      else kc = 200 * (i - j);
      ...
      for (const [surf, wc, pos, kind] of opts)
        for (const p of dp[j]) {
          ...
          const wd = (p.lastSurf != null && conn.wbi && conn.wbi[p.lastSurf + SEP + surf]) || 0; // 著者表記bigram割引
          acc.push({ cost: p.cost + wc + 100 + pen + cc - cb - wd, out: p.out + surf, segs: p.segs.concat([[sub, surf]]), lastKind: kind, lastPos: pos, lastSurf: surf });
        }
    }
...
// e2e/計測用: 文脈 ctx の直後に未確定かな列 yomi を置いた状態の変換計画を返す(状態は復元)。
// LLM審査は通さない=純粋な決定論ラティスの出力だけを観測する。
globalThis.__neConv = (yomi, ctx = '') => {
  const sv = { text, cursor, committedTo, mode, tut };
  try {
    tut = null; mode = 'NONE';
    text = ctx + yomi; cursor = text.length; committedTo = ctx.length;
    const plan = planConversion();
    return plan ? { kind: plan.kind, yomi: plan.yomi, list: plan.list } : null;
  } finally {
...
function render() {
  ...
  const showChart = tut || chartOn || beginner; // 練習・初心者は強制表示
  ...
  if (typeof document.body?.classList?.toggle === 'function') {
    ...
    document.body.classList.toggle('beginner', beginner); // 初心者モード(盤デカ表示)
  }
  const el = document.getElementById('text');
  ...
  if (beginner) { renderBeginner(el); return; }
  el.classList.remove('beginner-line');
...
async function main() {
  ...
  conn = await fetch('./conn.json').then((r) => r.json()).catch(() => ({})); // 連接モデル(任意・無くても動く)
  rebuildSelfPred();
  ...
  if (tocEl && tocEl.addEventListener) tocEl.addEventListener('click', (ev) => {
    ...
    if (sp && sp.dataset) { // 見開きへジャンプ
      moveCursor(Number(sp.dataset.pos));
      if (tategaki) { viewSpread = Number(sp.dataset.sp); render(); }
      return;
    }
