...
let symJump = 0;
let abcMode = false; // F11(本命)または英数キー(Lang2/無変換)でトグル。刻印どおりのQWERTYを素通し。英数はOSの入力ソース切替に取られる環境がある // 句読点を透かして変換した時、確定後にカーソルを記号の後ろへ戻す量
...
function onKeydown(e) {
  ...
  if (code === 'F11' && !tut) { e.preventDefault(); if (!e.repeat) abcToggle(); return; } // F11=ABCトグル(英数が取られる環境の本命)
...
function abcToggle() {
  ...
  status(abcMode ? 'ABC入力(F11/英数で戻る)' : 'かな入力に戻りました');
  render();
}
function onKeyup(e) {
